...
@section('content')
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header bg-transparent">
                    <h5 class="modal-title">Form Masterdata Luminaire Types</h5>
                ...
                <div class="modal-body px-3 pb-2">
                    ...
                    <form id="frm" class="row gy-1 gx-2">
                        @csrf
                        <input type="hidden" name="ms_lum_types_id" id="ms_lum_types_id">
                        <div class="col-12">
                            <label class="form-label" for="modalAddCardNumber">Luminaire Type Name</label>
                            <div class="input-group input-group-merge">
                                <input id="ms_lum_types_name" name="inp[ms_lum_types_name]" class="form-control add-credit-card-mask"
                                    type="text" />
                            </div>
                        </div>
    ...
    <input id="lumtype_id" type="hidden" />
@endsection
    ...
    <script>
        ...
        $(function() {
            ...
            dTable = $('#table').DataTable({
                ajax: {
                    url: "{{ url('api/masterdata/lumtypes/dt') }}",
                    type: 'post',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    }
                ...
                columns: [{
                    ...
                    {
                        data: 'ms_lum_types_name',
                        name: 'ms_lum_types_name'
                    },
                ],
                order: [
                    [1, 'desc']
        ...
        function addnew() {
            $('#frmbox').modal('show');
            $('#frm')[0].reset();
            $('#frm input[name="inp[ms_lum_types_id]"]').val('');
        }
        ...
        function save() {
            if ($("#frm").valid()) {
                ...
                $.ajax({
                    url: '{{ url('api/masterdata/lumtypes') }}',
                    type: 'post',
                    data: formData,
        ...
        function edit(id) {
            $.ajax({
                url: '{{ url('api/masterdata/lumtypes/') }}' + '/' + id,
                type: 'get',
                dataType: 'json',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
